...
<div class="add-rooms">
  ...
  <form action="${pageContext.request.contextPath}/addRoom" method="post" enctype="multipart/form-data">
	...
	<div class="form-row">
       <div class="form-fields">
         <label for="roomFloorNumber" class="label-for-form">Room Floor Number:</label>
          <input type="text" class="form-control ${not empty roomFloorError ? 'is-invalid' : ''}" id="roomFloorNumber" name="roomFloorNumber" value="${param.roomFloorNumber}" placeholder="${not empty roomFloorError ? roomFloorError : 'Enter room floor'}">
         <c:if test="${not empty roomFloorFormatError}">
		<small class="text-danger">${roomFloorFormatError}</small>
      </c:if>
       </div>

       <div class="form-fields">
         <label for="maxOccupancy" class="label-for-form">Max Occupancy:</label>
          <input type="text" class="form-control ${not empty maxOccupancyError ? 'is-invalid' : ''}" id="maxOccupancy" name="maxOccupancy" value="${param.maxOccupancy}" placeholder="${not empty maxOccupancyError ? maxOccupancyError : 'Enter room max members size'}">
         <c:if test="${not empty maxOccupancyFormatError}">
		<small class="text-danger">${maxOccupancyFormatError}</small>
      </c:if>
       </div>
     </div>
